...
*
  ...
  public function Adddetailbug($data)
  {
...
*
    ...
    $this->db->query('UPDATE sale_list_header_bak
    SET price_vat_all=price_vat_all + ' . $price_vat . ' WHERE sale_runno="' . $data['sale_runno'] . '"');
  }
...
WHERE product_id="' . $data['product_id'] . '" LIMIT 1),"0") as product_stock_num');
  ...
  public function Delshowcus($data)
  {
    ...
    // check if no product_name ==============
    if ($data['product_name'] == '') {
      $this->db->query('SELECT *,"' . $_SESSION['name'] . '","' . time() . '","' . $_SESSION['name'] . '","' . $_SESSION['shift_id'] . '"
FROM sale_list_cus2mer
WHERE user_id="' . $_SESSION['user_id'] . '"');

      $this->db->query('DELETE FROM sale_list_cus2mer  WHERE user_id="' . $_SESSION['user_id'] . '"');
      // $this->db->query('UPDATE  sale_list_cus2mer set sale_status="3" WHERE user_id="' . $_SESSION['user_id'] . '"  and sale_status="0"');
    }

    // else have product_name
    else {
      $this->db->query('SELECT *,"' . $_SESSION['name'] . '","' . time() . '","' . $_SESSION['name'] . '","' . $_SESSION['shift_id'] . '"
FROM sale_list_cus2mer
WHERE product_name="' . $data['product_name'] . '" and  user_id="' . $_SESSION['user_id'] . '" ');
      ...
      // $this->db->query('UPDATE sale_list_cus2mer set sale_status="3" WHERE product_name="' . $data['product_name'] . '" AND user_id="' . $_SESSION['user_id'] . '" and sale_status="0"');
    }
...
product_score
    FROM sale_list_cus2mer
    WHERE user_id="' . $_SESSION['user_id'] . '"     GROUP BY product_name
  ORDER BY sc_ID ASC
  ');
...
product_id,
  ...
  public function Delshowcusnotsum($data)
  {
    ...
    // $query = $this->db->query('UPDATE sale_list_cus2mer set sale_status="1"  WHERE sc_ID="' . $data['sc_ID'] . '" and sale_status="0"');
